...
| Eurostat Comext SDMX (`ec.europa.eu/eurostat/api/comext/dissemination/sdmx/2.1`) | `trade_flow` (EU-side alternate/supplement) | ⚠️ PROBE, leaning GO | Live-verified: `/dataflow` returns real SDMX metadata, no auth. `DS-045409` = "EU trade since 1988 by HS2-4-6 and CN8" — exactly the right shape. A data query needs the exact dimension order from the dataset's DSD (tried a guessed key `M.DE.US.TOTAL.VALUE_IN_EUROS`, got `INVALID_QUERY_NB_FILTERS`) — same "probe the real call" pattern as other Eurostat sources in this repo. Next step for whoever builds it: fetch `.../datastructure/ESTAT/DS-045409` first. |
