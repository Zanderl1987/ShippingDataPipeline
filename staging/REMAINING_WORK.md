...
| FreightPulse disruptions retired | Reported 0 rows every run: alerts moved from `data.alerts` to `data.results`, so nothing was ever written. The feed is now NWS US weather + GDACS/USGS natural hazards with no shipping impact fields; GDACS is already in `disruption_events`. Collector and tests deleted; `supply_chain_disruptions` schema kept (empty). See DATA_SOURCES.md 3.7. | ✅ Retired 2026-09-24 |
| One failed collector no longer blocks the publish | `collect_all` exits 2 (`EXIT_PARTIAL`) when the run finished but some collectors or curation failed; the Collect step turns that into `partial=true`, the HF sync runs, and a final step fails the job. A crash (any other exit) or a failed seed still stops the run before publishing. | ✅ Done 2026-09-24 |
| Curation validation fixed | Failed every run (1/3). `not_null(mmsi/timestamp)` demanded them from Axiomancer, which never sends either; AIS "not available" codes (sog 102.3, cog 360, heading 511) were stored as real values; 26 old AISStream/TankerMap rows lacked timestamp/mmsi; 3 UN/LOCODE ports had longitudes >180 (source typos). New curation step `cleaning.py` (before validation) NULLs invalid values and deletes non-Axiomancer rows without mmsi/timestamp; UN/LOCODE parser drops impossible coordinates; checks exempt Axiomancer, sog limit 102.2, new heading check. On the published data: 3,418 values nulled, 26 rows deleted, 3 ports fixed, 7/7 + 3/3 checks pass. | ✅ Done 2026-09-24 |
| PortWatch chokepoint paging fixed | `imf_portwatch.py` paged ArcGIS with no order and assumed 1,000-row pages. Now uses `portwatch_ports.query_all` (ordered by ObjectId, follows `exceededTransferLimit`, retries, raises on ArcGIS error bodies). Also: chokepoint-id filter bracketed so the date filter applies to every id; removed a hardcoded name map whose keys never matched and whose names were wrong (names come from PortWatch's `portname`); epoch dates parsed as UTC. Live: 78,960 rows = service count, no duplicates; published data already matched, so no history was affected. | ✅ Done 2026-09-24 |
| NOAA MarineCadastre rebuilt | The collector's URL layout never existed (404 back to 2024-01), so it wrote nothing since July while reporting success. Now reads NOAA's monthly track files from Azure into `vessel_tracks_us` (all vessel types, no geometry). First CI run backfills 2024-01 → 2025-12 (~30M rows, ~810 MB on HF). HF export switched to ZSTD compression. See DATA_SOURCES.md 1.7. | ✅ Built 2026-09-24 |
