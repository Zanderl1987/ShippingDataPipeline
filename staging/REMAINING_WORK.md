...
| USCG PSIX — new unvetted lead for `vessel_registry`/`vessel_safety` | `cgmix.uscg.mil` is a live ASP.NET site (200 OK) exposing vessel particulars + PSC exam/detention data for foreign vessels boarding in the US — would cover both empty priority tables from one source. `PSIXData.asmx?WSDL` redirected to a validation-error page on a plain GET, so it's SOAP-only and needs a real SOAP client (not a simple REST probe) to determine reachability. **Not vetted past this point** — genuine unknown, not a GO or a NO-GO yet. | ⚠️ PROBE — needs a SOAP client spike |

---

*Last updated: 2026-08-27 (Session 19 — CI lint fix, EIA reactivated, FreightPulse TLS diagnosed as NO-GO-for-now, HF upload restructured, USCG PSIX lead identified)*
