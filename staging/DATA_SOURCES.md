...
| **Collector** | `src/collectors/fred_oil.py` — `collect_oil_prices()`. Since 2026-09-25 it uses the keyless chart download `fredgraph.csv?id=DCOILWTICO,DCOILBRENTEU` (one CSV, both series, "" for no price), so no `FRED_API_KEY` is needed; it had never run in CI for lack of the key. An empty table is filled with the full history (1986→, ~10K rows, ~230 KB) in CI; otherwise the last 30 days. Live-verified 2026-09-25. |
...
| **Collector** | `freightpulse_fuel.py` → `fuel_prices`. Port congestion, freight rates, carriers and disruptions collectors **retired 2026-09-24** (see Verdict); their tables keep pre-change history (`supply_chain_disruptions` never held any rows). |
| **Verdict** | **PARTIAL** — only fuel prices still collected. **2026-09-24: API changed, three endpoints retired.** `port-congestion` now needs a port/country/region and its payload names **IMF PortWatch** as source: 7-day avg port calls vs 90-day avg (window includes the 7 days), no wait/anchorage data. Reproduced exactly from our `port_activity` (Rotterdam, Singapore, Istanbul, Oita), so it is rebuilt in curation as `port_congestion_proxy` for all ports at no API cost. `freight-rates` is now US trucking between two zips and returned `available: false` ("Error querying Warp freight API"); `carriers` is a US trucking name/DOT search. Neither has ocean data any more. `disruptions` moved its alerts from `data.alerts` to `data.results`, so the collector parsed 0 rows every run and never wrote anything; the alerts are now repackaged NWS (US-only weather, ~97%), GDACS and USGS natural hazards, with no route/transit/rate impact fields, and the API itself says it does not cover port closures, strikes or canal disruptions. GDACS events are already in PortWatch `disruption_events`. Retired rather than re-parsed. |
...
| **Collector** | `src/collectors/eurostat_comext.py` — `collect_comext_data(reporter=...)`, annual freq, product=TOTAL, both import/export flows, partner wildcarded. Live-verified 2026-08-28: `collect_comext_data(reporter="DE")` wrote 2,439 rows (253 partners, 2021-2025, both flows). |
| **Verdict** | **GO — BUILT.** Wired into `collect_all.py` as `eurostat_comext` (monthly schedule, reporter="DE" for now). Writes to the shared `trade_flow` table alongside UN Comtrade; migration `202608280001` widened `reporter_code`/`partner_code` to VARCHAR (Comext uses ISO-alpha codes, Comtrade uses numeric UN M49) and added a `currency` column (Comext is EUR, not USD like Comtrade's `trade_value_usd` column name implies — tagged rather than mislabeled). |
